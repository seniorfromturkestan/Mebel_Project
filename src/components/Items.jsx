...
const Modal = ({ message, onClose }) => (
  <div className="fixed top-0 left-0 w-full h-full bg-black bg-opacity-50 flex justify-center items-center z-50">
    <div className="bg-white p-6 md:p-8 rounded-lg shadow-lg text-center max-w-sm sm:max-w-md md:max-w-lg">
      ...
      <button
        onClick={onClose}
        className="mt-4 bg-gray-600 text-white rounded-lg text-base sm:text-lg px-4 py-3 md:py-2 w-full sm:w-auto cursor-pointer hover:bg-opacity-90 transition duration-200 active:bg-slate-300"
      >
        Закрыть
      </button>
    </div>
...
const Items = ({ items = [], addToCart, searchQuery = '', toggleFavorite, favorites = [], isAuthenticated }) => {
  ...
  const filteredItem = filteredItems.filter(item => item.title?.toLowerCase().includes(searchQuery.toLowerCase()));
    ...
    <div className="wrapper">
      ...
      <div className="flex md:flex-wrap space-x-3 my-5 pl-3 xl:pl-0 overflow-x-auto max-h-12 md:max-h-20 scrollbar-hide">
        {categories.map(category => (
          <button key={category} onClick={() => setSelectedCategory(category)} className={`px-4 py-2 rounded-lg ${selectedCategory === category ? 'bg-gray-600 text-white' : 'bg-gray-200 text-black'} transition duration-300`}>
          ...
          filteredItem.map(item => (
            <div key={item.id} className="w-full relative mb-3">
              <div className="rounded-lg shadow-lg h-[230px] sm:h-[290px] lg:h-[300px] overflow-hidden mb-3">
                ...
                <div onClick={() => handleToggleFavorite(item.id)} className="text-2xl md:text-3xl absolute right-3 bottom-30 text-gray-600 cursor-pointer hover:opacity-80 transition duration-200">
                  {favorites.includes(item.id) ? <FaHeart className="text-red-500" /> : <FaRegHeart />}
                </div>
              </div>
              <div className="w-full pr-1 text-[#333] space-y-3 pt-1">
                <Link to={`/${item.id}`} className="font-semibold py-4 text-gray-600 text-xl sm:text-2xl cursor-pointer w-fit">
                  {item.title}
                </Link>
      ...
      {showModal && <Modal message="Ваш товар успешно добавлен в корзину!" onClose={() => setShowModal(false)} />}
    </div>
  );
};
